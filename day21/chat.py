...
class ConsoleChat:
    ...
    def display_welcome(self):
        ...
        self.console.print("RAG with Ollama Embeddings + Reranking:", style="yellow")
        ...
        self.console.print("  /compare <question> - Compare WITH/WITHOUT reranking", style="bright_green")
    ...
    def _init_rag_components(self):
        ...
        if self.rag_comparator is None:
            ...
            # Use Ollama for Koog embeddings (1024d)
            ...
            self.rag_comparator = RagComparator(
                ...
                embedding_generator=ollama_gen,  # Use Ollama for query embeddings
    ...
    def load_koog(self, force: bool = False):
        ...
        try:
            ...
            if koog_exists and not force:
                ...
                self.console.print("💡 Try: /koog-info or /compare <question>", style="bright_yellow")
                return
        ...
        try:
            ...
            self.console.print("\n💡 Try: /koog-info or /compare <question>", style="bright_yellow")
    ...
    def compare_rag(self, question: str):
        ...
        try:
            ...
            # Display RAG WITHOUT reranking
            self.console.print()

            # Render answer as Markdown
            without_answer_md = Markdown(result.answer_without_reranking)

            # Build chunks info as Rich Text
            without_chunks_info = Text()
            without_chunks_info.append(f"\nRetrieved {len(result.chunks_without_reranking)} chunks (direct vector search):\n", style="yellow")
            for idx, chunk in enumerate(result.chunks_without_reranking, 1):
                source_name = Path(chunk.source).name
                without_chunks_info.append(f"  {idx}. {source_name} (chunk {chunk.chunk_index}, distance: {chunk.distance:.4f})\n")
            without_chunks_info.append(f"\nTime: {result.time_without_reranking:.2f}s | Tokens: ~{result.tokens_without_reranking}", style="dim")

            self.console.print(Panel(
                Group(without_answer_md, without_chunks_info),
                title="RAG WITHOUT RERANKING (no filter)",
                border_style="yellow",
                padding=(0, 2)
            ))

            # Display RAG WITH reranking
            # Render answer as Markdown
            answer_md = Markdown(result.answer_with_reranking)

            # Build chunks info as Rich Text
            chunks_info = Text()
            chunks_info.append(f"\nRetrieved {len(result.chunks_with_reranking)} chunks (distance filter + Gemini scoring):\n", style="green")
            for idx, chunk in enumerate(result.chunks_with_reranking, 1):
                source_name = Path(chunk.source).name
                rerank_info = f", rerank: {chunk.rerank_score:.1f}/10" if chunk.rerank_score else ""
                chunks_info.append(f"  {idx}. {source_name} (chunk {chunk.chunk_index}, distance: {chunk.distance:.4f}{rerank_info})\n")
            chunks_info.append(f"\nTime: {result.time_with_reranking:.2f}s | Tokens: ~{result.tokens_with_reranking}", style="dim")

            with_panel = Panel(
                Group(answer_md, chunks_info),
                title="RAG WITH RERANKING (filtered)",
                border_style="green",
                padding=(0, 2)
            )
            self.console.print(with_panel)

            # Conclusion
            self.console.print("\n" + "=" * 70, style="bright_cyan")
            self.console.print("💡 Analysis:", style="bright_yellow bold")
            self.console.print(f"  • Reranking filtered from {len(result.chunks_without_reranking)} to {len(result.chunks_with_reranking)} chunks", style="dim")
            self.console.print(f"  • Response time difference: {result.time_with_reranking - result.time_without_reranking:+.2f}s", style="dim")
            self.console.print(f"  • Token difference: {result.tokens_with_reranking - result.tokens_without_reranking:+} tokens", style="dim")

            # Show reranking stats if available
            if result.reranking_stats:
                stats = result.reranking_stats
                self.console.print("\n📊 Reranking Statistics:", style="bright_yellow bold")
                self.console.print(f"  • Initial candidates: {stats['initial_count']}", style="dim")
                self.console.print(f"  • After distance filter: {stats['after_distance_filter']}", style="dim")
                self.console.print(f"  • After rerank filter: {stats['after_rerank_filter']}", style="dim")
                self.console.print(f"  • Final selected: {stats['final_count']}", style="dim")
                self.console.print(f"  • Reranking time: {stats['reranking_time']:.2f}s", style="dim")
