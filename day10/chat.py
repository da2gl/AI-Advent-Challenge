"""Console chat interface for Gemini API with Multi-MCP integration."""
...
class ConsoleChatWithMCP:
    ...
    DEFAULT_SYSTEM_INSTRUCTION = (
        "You are an AI assistant with access to multiple tools through MCP. "
        "You can fetch web content and manage containers. "
    ...
    async def connect_to_mcp(self):
        ...
        # Try to connect to container server (local, no API key needed)
        try:
            self.console.print("\n[yellow]Connecting to Container MCP server...[/yellow]")
            spinner = Spinner("dots", text="Connecting...", style="yellow")
            with Live(spinner, console=self.console, transient=True):
                await self.mcp_manager.connect_client("container")

            client = self.mcp_manager.get_client("container")
            self.console.print("[green]✓ Connected to Container MCP server[/green]")
            self.console.print(f"[dim]Found {len(client.get_tools())} tools[/dim]")
        except Exception as e:
            self.console.print(f"[red]✗ Failed to connect to Container server: {str(e)}[/red]")

        # Try to connect to fetch server (no API key needed)
        try:
