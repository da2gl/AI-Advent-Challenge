...
# Disable ChromaDB telemetry (must be before importing chromadb-dependent modules)
...
# Custom stderr wrapper to suppress MCP cleanup errors
class FilteredStderr:
    ...
    def write(self, text):
        ...
        if self.suppress and ('async_generator' in text or 'cancel scope' in text or 'RuntimeError' in text):
            return
...
# Enable suppression on exit
def _enable_suppression():
    _filtered_stderr.suppress = True


atexit.register(_enable_suppression)

from core.gemini_client import GeminiApiClient, GeminiModel  # noqa: E402
...
class ConsoleChat:
    ...
    async def _process_with_tools(self, prompt: str, history: list) -> tuple:
        ...
        while self.gemini_client.has_function_calls(response) and turn_count < max_turns:
            turn_count += 1
        ...
        while True:
            try:
                ...
                with Live(spinner, console=self.console, transient=True):
                    ...
                    # Process with tools (async)
                    response_text, function_calls = asyncio.run(
                ...
                if function_calls:
                    self.console.print(f"\n\n🔧 Tools used ({len(function_calls)}):", style="cyan bold")
                    for idx, fc in enumerate(function_calls, 1):
                        self.console.print(f"  {idx}. {fc['name']}", style="cyan dim")
