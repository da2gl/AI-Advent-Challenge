...
class GeminiApiClient:
    ...
    def generate_content(
        ...
        # Add user message ONLY if prompt is not empty
        # This prevents adding empty user messages in multi-turn function calling
        if prompt:
            contents.append({
                "parts": [{"text": prompt}],
                "role": "user"
    ...
    def extract_text(self, response: Dict) -> str:
        ...
        try:
            ...
            # Extract and concatenate text from ALL parts (not just first one)
            # Gemini can split response into multiple parts
            text_parts = []
            for part in parts:
                if "text" in part:
