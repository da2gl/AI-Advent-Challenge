...
class MCPManager:
    ...
    async def connect_client(self, client_type: str) -> bool:
        ...
        Args:
            client_type: Type of client to connect ("browser" or "fetch")
    ...
    def get_all_tools_for_gemini(self) -> List[Dict[str, Any]]:
        ...
        for client in self.clients.values():
            if client.is_connected():
                all_functions.extend(client.convert_tools_to_gemini_format())
    ...
    async def call_tool(
        ...
        """Call a tool on a specific client or active client.

        Args:
            tool_name: Name of the tool to call
            arguments: Arguments to pass to the tool
            client_type: Type of client to use (None = use active)
        ...
        if client_type:
            ...
            if not client:
                raise Exception("No active MCP client")
